...
class NEWBendingCollegeWav2Vec(BendingCollegeWav2Vec):
    ...
    def description(self, sequence_len):
        ...
        desc += f"\n    - InfoNCE (a={self.alpha_infonce})"

        if self.use_cluster_loss:
            desc += f"\n    - Cluster Contrastive (a={self.alpha_cluster}, t={self.cluster_temp}, K={self.num_clusters})"

        if self.use_reconstruction_loss:
            desc += f"\n    - Reconstruction (a={self.alpha_recon})"

        desc += f"\n    - L2 Regularization (b={self.beta})"
